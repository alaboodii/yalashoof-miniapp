...
try {
  ...
  ts.onload = function(){
    ...
    var goFs = function(){
      if (requestedFs || tg.isFullscreen) return;
      try { if (typeof tg.requestFullscreen === "function") { tg.requestFullscreen(); requestedFs = true; return; } } catch(_){}
      try {
    ...
    setTimeout(goFs, 100);
    setTimeout(goFs, 500);
    setTimeout(goFs, 1500);
    setTimeout(goFs, 3000);
    document.addEventListener("touchstart", goFs, {passive:true, capture:true});
    document.addEventListener("click",      goFs, {capture:true});
    try { tg.lockOrientation && tg.lockOrientation(); } catch(_){}
    try { window.__YALA_TG_VER = tg.version || ""; } catch(_){}
  };
...
var STREAM_HOSTS = [
  ...
  // (soccertvhd / frekora / korasimo moved to DIRECT_HOSTS for native player loading)
];
...
var DIRECT_HOSTS = [
  ...
  // Stream iframe-player chain (anti-bot/Adscore detection requires direct origin)
  "yalla-sport.link","yalla-sport.com",
  "dynmaspect.net","toprevenuegate.com",
  "frekora.live"
];
